...
export const load = async ({ url }: { url: URL }) => {
    ...
    // Query for starred messages (those with is_starred = true)
    let query = supabaseAdmin
        .from("mail_messages")
        .select(
            "gmail_message_id,gmail_thread_id,from_email,to_emails,subject,snippet,internal_date,direction,account,label_ids,is_read,is_starred,is_trashed,is_archived",
        )
        .eq("is_starred", true)
        .eq("is_trashed", false)
