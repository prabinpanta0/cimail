...
<script lang="ts">
	...
	const buildAccountHref = (account: string) => {
		...
		return `/starred?${params.toString()}`;
	};
		...
		<main class="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0f1115]">
			...
			<div class="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-border-dark shrink-0 h-14 bg-white dark:bg-[#111318]">
				<div class="flex items-center gap-4">
					<span class="material-symbols-filled text-yellow-500">star</span>
					<h1 class="text-lg font-semibold text-gray-900 dark:text-white">Starred</h1>
				</div>
				<div class="text-sm text-gray-500 dark:text-gray-400">
					{data.totalCount ?? 0} starred
				</div>
			</div>
			...
			<EmailList
				...
				emptyMessage="No starred messages. Star important emails to find them here."
				{selectedIds}
